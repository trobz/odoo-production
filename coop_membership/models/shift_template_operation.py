...
class ShiftTemplateOperation(models.Model):
    ...
    def _mass_change_team(self, registrations, target_template):
        ...
        for reg in registrations:
            _logger.debug("Creating shift.change.team for %s", reg.partner_id.name)
        ...
        for template in self.template_ids:
            ...
            for i in range(0, self.quantity):
                ...
                start_date = next_date + relativedelta(
                    **{delay: mult * (self.offset * i)}
                )
                # New start/end datetimes
                start_datetime = template.start_datetime.replace(
                    year=start_date.year,
                    month=start_date.month,
                    day=start_date.day,
                )
                end_datetime = template.end_datetime.replace(
                    year=start_date.year,
                    month=start_date.month,
                    day=start_date.day,
                )
                ...
                original_start_dt_tz = self._datetime_as_in_tz(template.start_datetime)
    ...
    def _execute_create_and_move(self):
        ...
        for template in self.template_ids:
            ...
            chunked_registrations = list(
                split(template.current_registration_ids, self.quantity)
            )
            # Mass move them
            for i in range(0, self.quantity):
                self._mass_change_team(
    ...
    def _execute_move_back(self):
        ...
        if missing_original_template_ids:
            raise UserError(
                ...
                % missing_original_template_ids.ids
            )
        # Group by original template and move one by one
        original_templates = self.template_ids.mapped("original_shift_template_id")
    ...
    def execute(self):
        ...
        if self.strategy == "move to":
            ...
            raise UserError(_("Strategy '%s' not implemented!") % self.strategy)
        # Update state

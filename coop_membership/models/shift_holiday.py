...
class ShiftHoliday(models.Model):
    ...
    def check_over_lap(self):
        for record in self:
            if record.date_begin > record.date_end:
                raise ValidationError(_("Date End should be greater than Date Begin."))
            elif (
                record.holiday_type == "single_day"
                and record.date_end != record.date_begin
            ):
        ...
        for record in self:
            ...
            if record.holiday_type == "long_period":
                shifts = shifts.filtered(
                    lambda s: not s.long_holiday_id
                    or s.long_holiday_id.state == "cancel"
                )
                record.long_period_shift_ids = [[6, 0, shifts.ids]]
            else:
                shifts = shifts.filtered(
                    lambda s: not s.single_holiday_id
                    or s.single_holiday_id.state == "cancel"
                )
    ...
    def attribute_point_qty_long_period(self):
        ...
        for attendee in attendees:
            ...
            if self.make_up_type == "0_make_up":
                if not attendee.shift_id.shift_type_id.is_ftop:
                    if attendee.template_created:
                        if attendee.state == "absent":
                            count_vals = {"point_qty": 2}
                        elif attendee.state == "excused":
                            count_vals = {"point_qty": 1}
                    elif (
                        not attendee.template_created
                        and attendee.shift_type == "ftop"
                        and attendee.state == "absent"
                    ):
                        count_vals = {"point_qty": 1}
                    ...
                    if attendee.shift_id.state == "done":
                        if attendee.partner_id.final_ftop_point >= 0:
                            ...
                            if (
                                ...
                                and attendee.reduce_extension_id.reduce_deduction
                            ):
                                count_vals = {"point_qty": 1}
                            else:
                                count_vals = {"point_qty": 2}
                ...
                if (
                    not attendee.shift_id.shift_type_id.is_ftop
                    and attendee.state == "absent"
                    and attendee.template_created
                ) or (
                    attendee.shift_id.shift_type_id.is_ftop
                    and attendee.shift_id.state == "done"
                    and attendee.partner_id.final_ftop_point < -1
                ):
                    count_vals = {"point_qty": 1}
            if count_vals:
                count_vals.update(
    ...
    def attribute_point_qty_single_day(self):
        ...
        another_holiday = self.single_day_shift_ids.filtered(
            lambda s: s.long_holiday_id and s.long_holiday_id.state == "done"
        ).mapped("long_holiday_id")
            ...
            if not attendee_closed.shift_id.shift_type_id.is_ftop:
                if attendee_closed.template_created:
                    if attendee_closed.state == "absent":
                        count_vals_closed = {"point_qty": 2}
                    elif attendee_closed.state == "excused":
                        count_vals_closed = {"point_qty": 1}
                elif (
                    not attendee_closed.template_created
                    and attendee_closed.shift_type == "ftop"
                    and attendee_closed.state == "absent"
                ):
                    count_vals_closed = {"point_qty": 1}
                ...
                if attendee_closed.shift_id.state == "done":
                    if attendee_closed.partner_id.final_ftop_point >= 0:
                        ...
                        if (
                            ...
                            and attendee_closed.reduce_extension_id.reduce_deduction
                        ):
                            count_vals_closed = {"point_qty": 1}
                        else:
                            count_vals_closed = {"point_qty": 2}
        ...
        for attendee_open in attendee_in_open_shifts:
            if (
                attendee_open.state == "absent"
                and attendee_open.template_created
                and not attendee_open.shift_id.shift_type_id.is_ftop
            ) or (
                attendee_open.shift_id.shift_type_id.is_ftop
                and attendee_open.shift_id.state == "done"
                and attendee_open.partner_id.final_ftop_point < -1
            ):
                count_vals_open = {
    ...
    def cron_update_shifts(self):
        records = self.search([("state", "not in", ("draft", "cancel"))])

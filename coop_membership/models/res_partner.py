...
class ResPartner(models.Model):
    ...
    def _check_number_of_associated_people(self):
        ...
        avail_check = config_param_env.sudo().get_param(key_avail_check, "unlimited")
        for rec in self:
            if (
                avail_check == "limited"
                and rec.is_member
                and rec.nb_associated_people > max_nb
            ):
                raise ValidationError(
    ...
    def _compute_badge_to_distribute(self):
        for record in self:
            ...
            if record.badge_print_date:
                if (
                    not record.badge_distribution_date
                    or record.badge_distribution_date < record.badge_print_date
                ):
    ...
    def _compute_total_partner_owned_share(self):
        for partner in self:
            partner.total_partner_owned_share = sum(
                partner_ownedshare.owned_share
    ...
    def _compute_number_of_associated_people(self):
        for partner in self:
            if (partner.is_member or partner.is_former_member) and partner.child_ids:
                partner.nb_associated_people = sum(
                    [
                        (p.is_associated_people or p.is_former_associated_people)
                        and 1
                        or 0
    ...
    def _compute_current_template(self):
        for partner in self:
            ...
            if reg:
                ...
                reg = partner.tmpl_reg_ids.filtered(lambda r: r.is_future)
                if reg:
                    current_template = reg[0].shift_template_id
            if current_template:
    ...
    def write(self, vals):
        ...
        if not (
            ...
            or "barcode" in vals
            or "barcode_rule_id" in vals
            or "property_account_payable_id" in vals
        ):
            for partner in self:
                self._generate_associated_barcode(partner)
        # Recompute display_name if needed
        if "name" not in vals and ("barcode_base" in vals or "is_member" in vals):
            for partner in self:
    ...
    def check_designated_buyer(self, partner):
        ...
        if partner.is_designated_buyer:
            partner.deactivated_date = today + relativedelta(days=maximum_active_days)
    ...
    def _generate_associated_barcode(self, partner):
        ...
        if (
            not partner.is_member
            and partner.is_associated_people
            and not partner.barcode_rule_id
    ...
    def update_welcome_email_manually(self, limit=None):
        ...
        partners.write({"welcome_email": True})

    @api.model
    def deactivate_designated_buyer(self):
        partners = self.search([("deactivated_date", "=", fields.Date.today())])
    ...
    def cron_send_notify_mirror_children_email(self):
        ...
        """
        self.env.cr.execute(query, (today_dt_last_18years_tr,))
    ...
    def _update_when_number_of_shares_reaches_0(self):
        ...
        for partner_share in self.partner_owned_share_ids:
            invoice_states += [
                invoice.state in ["open", "paid", "cancel"]
        ...
        if (
            ...
            for tmpl_reg in self.tmpl_reg_line_ids:
                if not tmpl_reg.date_end or tmpl_reg.date_end > fields.Date.today():
    ...
    def create_related_user(self):
        ...
        if partner.email and not partner.user_ids:
            ...
            vals = {"login": partner.email, "partner_id": partner.id}

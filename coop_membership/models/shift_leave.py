...
class ShiftLeave(models.Model):
    ...
    absence_less_than_15days = fields.Boolean(
        compute="_compute_absence_less_than_15days", store=True
    )
    ...
    def _compute_proposed_date(self):
        ...
        for leave in self:
            ...
            if (
                not (leave.partner_id and leave.type_id and leave.stop_date)
            ...
            _next_shift_time, next_shift_date = partner.get_next_shift_date(
            ...
            if next_shift:
                shift_name = next_shift.name + (
                    next_shift.begin_date_string
                    and (" " + next_shift.begin_date_string)
                    or ""
                )
            else:
                format_next_shift_date = (
                    next_shift_date and next_shift_date.strftime(date_format) or ""
                )
            ...
            format_stop_date = stop_date and stop_date.strftime(date_format) or ""
            leave.alert_message = alert_message.format(
                end_date=format_stop_date,
                partner=leave.partner_id.name,
                proposed_date=format_proposed_date,
                shift=shift_name,
            )
    ...
    def _compute_shift_time_after_return(self):
        ...
        for leave in self:
            ...
            leave_return_time_utc = self.convert_date_to_utc_datetime(leave.return_date)

            # Look for date next shift after return date
            _next_shift_time, next_shift_date = leave.partner_id.get_next_shift_date(
    ...
    def convert_date_to_utc_datetime(self, input_date, start_of_day=True):
        ...
        """
        if start_of_day:
            input_date = str(input_date) + " 00:00:00"
        else:
            input_date = str(input_date) + " 23:59:59"
    ...
    def _compute_proceed_message(self):
        for leave in self:
            if leave.stop_date and leave.start_date:
                days_leave = (
                    datetime.strptime(str(leave.stop_date), "%Y-%m-%d")
                    - datetime.strptime(str(leave.start_date), "%Y-%m-%d")
                ).days + 1
    ...
    def calculate_number_shift_future_in_leave(self):
        ...
        for template in templates:
            ...
            rec_dates = template.get_recurrent_dates(last_shift_date, self.stop_date)
            ...
            if leave.non_defined_type and leave.non_defined_leave:
                ...
                if last_templates:
                    ...
                    for line in last_templates:
                        # date_end must meet the template constraint
                        # (end date >= begin date), so never set it before
                        # the line's begin date
                        date_end = leave_start - timedelta(days=1)
                        if date_end < line.date_begin:
                            date_end = line.date_begin
    ...
    def send_mail_reminder_non_defined_leaves(self):
        ...
        mail_template = self.env.ref("coop_membership.reminder_end_leave_email")
        if mail_template:
    ...
    def cron_update_shift_leader_end_leave(self):
        ...
        yesterday_dt = fields.Date.from_string(fields.Date.today()) - timedelta(days=1)
        yesterday_str = fields.Date.to_string(yesterday_dt)
        end_leaves = self.search([("stop_date", "=", yesterday_str)])

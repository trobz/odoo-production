...
class ShiftRegistration(models.Model):
        ...
        for rec in self.sudo():
            if rec.shift_id.date_begin:
                ...
                for next_reg in next_regs.sorted("date_begin"):
                    self.check_registration_period(next_reg.date_begin, rec.partner_id)
    ...
    def button_reg_absent(self):
        ...
        for reg in self:
            partner_cooperative_states[reg.partner_id] = (
                reg.partner_id.cooperative_state
            )
        res = super().button_reg_absent()
        for reg in self:
            ...
            if reg.template_created and reg.shift_type == "standard":
                ...
                if last_shift_reg and last_shift_reg.state == "absent":
                    ...
                    if not markup_shift_reg_count:
                        ...
                        utc_timestamp = pytz.utc.localize(reg.date_end, is_dst=False)
                        ...
                        mail_template = self.env.ref(template_name)
    ...
    def _get_reference_counter_point_qty(
        self, shift_type, registration_state, custom_reference_points=None
    ):
        """
        ...
        default_reference_points = {
            ...
            "standard": {"done": 1, "replaced": 1, "absent": -2, "excused": -1},
        }
    ...
    def write(self, vals):  # noqa: C901
            ...
            if vals_state != shift_reg.state:
                ...
                if shift_reg.shift_type == "ftop":
                    if vals_state in ["done", "replaced"]:
                        reason = vals_state == "done" and _("Attended") or _("Replaced")
                        ...
                        counter_vals["point_qty"] = (
                            self._get_reference_counter_point_qty(
                        ...
                        if (
                            not shift_reg.tmpl_reg_line_id
                            and shift_reg.partner_id.in_ftop_team
                        ):
                            counter_vals["ignored"] = True

                    elif vals_state in ["excused"] and shift_reg.template_created:
        ...
        if (
            "template_created" in vals
            or "shift_ticket_id" in vals
            or "shift_id" in vals
        ):
        ...
        if vals in ["absent", "excused"]:
            ...
            if self.shift_type == "standard" and self.template_created:
                if single_holiday:
                    ...
                    elif self.shift_id.state_in_holiday == "open" and vals in [
                        "absent"
                    ]:
                        counter_vals.update({"point_qty": 1})
                elif long_holiday:
                    if long_holiday.make_up_type == "0_make_up":
                        if vals in ["absent"]:
                            ...
                            counter_vals.update({"point_qty": 1})
                    elif long_holiday.make_up_type == "1_make_up" and vals in [
                        "absent"
                    ]:
                        counter_vals.update({"point_qty": 1})
                ...
                if single_holiday:
                    ...
                    if long_holiday.make_up_type == "0_make_up":
                        if (vals in ["absent"]) or (
                            ...
                            counter_vals.update({"point_qty": 1})
            if counter_vals:
                holiday_id = single_holiday and single_holiday.id or long_holiday.id
                counter_vals.update(
                    {
    ...
    def balance_point_qty_ftop_shift(self, holiday_id, current_point, state):
        ...
        if current_point >= 1:
            ...
            if state in ["0_make_up", "closed"]:
                if (
                    self.reduce_extension_id
                    and self.reduce_extension_id.reduce_deduction
                ):
                    point = 1
                else:
                    point = 2
    ...
    def onchange_shift_id(self):
        ...
        if is_standard_ticket:
            ticket_type_product = self.env.ref(
                "coop_shift.product_product_shift_standard"
            )
        else:
            ticket_type_product = self.env.ref("coop_shift.product_product_shift_ftop")
        for reg in self:
    ...
    def _compute_display_name(self):
        for registration in self:
            ...
            registration.display_name = shift_name + (
                " " + date_begin if date_begin else ""
            )
    ...
    def checking_shift_attendance(self):
        ...
        @Function to check the attendance:
            ...
            in a ABCD shift on a ABCD tickets
        """
        ignore_checking = self.env.context.get("ignore_checking_attendance", False)
        if ignore_checking:
            return
        uptodate_list = []
        for shift_reg in self:
            shift_type = shift_reg.shift_id and shift_reg.shift_id.shift_type_id
            if (
                shift_type
                and not shift_type.is_ftop
                and shift_reg.shift_type == "standard"
                and not shift_reg.template_created
                and shift_reg.state not in ["replacing", "replaced"]
                and shift_reg.exchange_state not in ["replacing", "replaced"]
        ...
        if uptodate_list:
            raise UserError(
                _(
                    "Warning! You cannot enter make-ups of "
    ...
    def check_leave_time(self):
        ...
        for reg in self.sudo():
            ...
            for leave in leaves:
                if (
                    reg.date_end.date() >= leave.start_date
                    and reg.date_begin.date() <= leave.stop_date
                ):
                    if reg.shift_id.shift_type_id.is_ftop:
                        ...
                        if is_from_template:
                            ...
                            # Get list attendee
                            partner_tmp_ids = (
                                reg.shift_id.shift_template_id.registration_ids.mapped(
                                    "partner_id"
                                ).ids
                            )
                        ...
                        and leave.type_id.is_temp_leave
                    ):
                        # Check attendee be crated from template
                        if is_from_template:
                            reg.state = "waiting"

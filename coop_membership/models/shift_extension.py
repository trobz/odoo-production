...
class ShiftExtension(models.Model):
    ...
    current_extension = fields.Boolean(
        compute="_compute_current_extension", store=False
    )
    ...
    def _compute_show_reduce_deduction(self):
        for record in self:
            ...
            if (
                record.type_id.extension_method == "to_next_regular_shift"
                and record.partner_id.shift_type == "ftop"
            ):
    ...
    def suggest_extension_date_stop(self, extension_type, partner, date_start):
        ...
        if extension_type and date_start:
            if extension_type.extension_method == "fixed_duration":
                ...
                date_stop = date_start + relativedelta(days=extension_type.duration)
            elif partner and extension_type.extension_method == "to_next_regular_shift":
                # Get next shift date
    ...
    def _validate_reduced_deduction(self):
        ...
        for record in self:
            ...
            shift_regs = shift_regs.filtered(
                lambda s: s.shift_id.state != "done"
            ).sorted(key=lambda shift: shift.date_begin)
            if shift_regs:
            ...
            if record.partner_id.display_ftop_points >= 1:
                raise ValidationError(_("The counter of the member is positive."))

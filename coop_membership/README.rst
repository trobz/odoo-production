...
- Replace ``o_cp_sidebar`` by ``actionMenuItems``: include
  ...
  (FormController)
- Replace ``o_button_import`` by ``cogMenuImport``: Import button in cog
  menu from ListController and KanbanController
- Replace ``o_chatter_topbar`` by ``o_mail_Chatter_top``: Chatter topbar
  in FormController

...
class CapitalFundraisingWizard(models.TransientModel):
    ...
    category_id = fields.Many2one(
        comodel_name="capital.fundraising.category", default=default_category_id
    )
    keep_partner_barcode = fields.Boolean(
        default=True, string="Keep existing barcode on Partner"
    )
    ...
    def button_confirm(self):
        ...
        if (
            ...
            and (
                not self.keep_partner_barcode
                or self.partner_id.is_member
                or self.partner_id.is_associated_people
            )
        ):

...
class ReportContractMemberAbstract(models.AbstractModel):
    ...
    def get_date_meeting(self, partner_id):
        ...
        event_reg_id = event_reg_env.search([("partner_id", "=", partner_id)], limit=1)

        if event_reg_id:
            return (
                event_reg_id.event_id.date_begin
                and event_reg_id.event_id.date_begin[:10]
                or False
            )
        else:
            return False
    ...
    def _get_report_values(self, docids, data=None):
        ...
        return {"docs": docs, "event_date": self.get_date_meeting(docids[0])}

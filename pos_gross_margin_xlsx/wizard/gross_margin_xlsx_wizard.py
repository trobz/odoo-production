...
class GrossMarginXlsxWizard(models.TransientModel):
    ...
    def export_report(self):
        ...
        return rp.report_action(self, data={})
    ...
    def get_datas(self):
        ...
        for categ in self.category_ids:
            products = self.env["product.product"].search([("categ_id", "=", categ.id)])
    ...
    def get_pre_tax_net_sales(self, categ, from_date, to_date):
        ...
        res = sum(datas.mapped("price_wo_tax")) if datas else 0.0
        return res

    def get_inventory_value(self, date, products):
        prods = products.with_context(to_date=date)
        res = sum(prods.mapped("value_svl")) if prods else 0.0
        return res

    def get_net_purchases(self, categ, from_date, to_date):
        if not categ:
            return 0.0
        from_date_dt = fields.Datetime.context_timestamp(self, from_date)
        from_date_str = from_date_dt.strftime(DF)
        to_date_dt = fields.Datetime.context_timestamp(self, to_date)
        to_date_str = to_date_dt.strftime(DF)

        query = SQL(
            """
            SELECT COALESCE(SUM(ail.price_subtotal), 0)

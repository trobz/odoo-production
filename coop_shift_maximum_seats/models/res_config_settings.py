...
class ResConfigSettings(models.TransientModel):
    ...
    def onchange_shift_max_available_seats(self):
        if self.shift_max_available_seats == "auto" and \
                self.shift_exchange_policy == "registraion_standard_ftop":
            self.shift_max_available_seats = "manual"
            return {"warning": {
                        "titile": _("Validation Error"),
                        "message": _("This option is not available when `Shift replacement list contains` is `Participations put into exchange plus available standard seats plus available ftop seats`")
            }}
    ...
    def onchange_shift_exchange_policy(self):
        if self.shift_max_available_seats == "auto" and \
            ...
            return {"warning": {
                        "titile": _("Validation Error"),
                        "message": _("This option is not available when `Maximum available ABCD/FTOP seats` is `Auto`")
            }}

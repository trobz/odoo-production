...
class TestCoopShift(common.TransactionCase):
    def setUp(self):
        ...
        self.ftop_template_2 = ModelData.xmlid_to_res_id("coop_shift.ftop_template_2")

        self.standard_member_1 = ModelData.xmlid_to_res_id(
            "coop_shift.standard_member_1"
        )

        self.ftop_member_1 = ModelData.xmlid_to_res_id("coop_shift.ftop_member")

        self.date_from = fields.Date.today()
        self.date_to = fields.Date.today() + timedelta(days=1)

        self.shift_type_id = ModelData.xmlid_to_res_id("coop_shift.shift_type")
    ...
    def test_standard_member_event_registration(self):
        ...
        wiz = self.ShiftWizard.create(
            {
                ...
                "template_ids": [(6, 0, {self.shift_template1})],
            }
        )
        ...
        shift_id = self.Shift.search(
            [("shift_template_id", "=", self.shift_template1), ("state", "=", "draft")]
        )
        ...
        self.assertEqual(
            shift_id.state, "confirm", "Shift: confirmation of shift failed"
        )
    ...
    def test_ftop_member_event_registration(self):
        ...
        wiz = self.ShiftWizard.create(
            {
                "date_from": self.date_from,
                "date_to": self.date_to,
                "template_ids": [(6, 0, {self.ftop_template_2})],
            }
        )
        ...
        shift_id = self.Shift.search(
            [("shift_template_id", "=", self.ftop_template_2), ("state", "=", "draft")]
        )

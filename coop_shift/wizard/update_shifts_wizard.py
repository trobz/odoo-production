...
class UpdateShiftsWizard(models.TransientModel):
    ...
    def _get_line_ids(self, template, date_from=None, date_to=None):
        ...
        for shift in template.shift_ids:
            if (
                (not date_from or shift.date_begin.date() >= date_from)
                and (not date_to or shift.date_end.date() <= date_to)
                and shift.state == "draft"
            ):
                line_ids.append(
                    (
                        0,
    ...
    def update_shifts(self):
        ...
        for wizard in self:
            if self.updated_fields and isinstance(safe_eval(self.updated_fields), dict):

...
class ReplaceRegistration(models.TransientModel):
    ...
    registration_id = fields.Many2one(
        "shift.registration",
        "Registration",
        default=_get_registration_id,
        required=True,
        ondelete="cascade",
    )
    shift_id = fields.Many2one(related="registration_id.shift_id", readonly=True)
    shift_ticket_id = fields.Many2one(
        string="Shift Ticket", related="registration_id.shift_ticket_id", readonly=True
    )
    ...
    partner_id = fields.Many2one(
        string="Replaced Member", related="registration_id.partner_id", readonly=True
    )
    new_partner_id = fields.Many2one("res.partner", "Replacing Member", required=True)

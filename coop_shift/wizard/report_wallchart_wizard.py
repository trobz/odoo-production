...
class ReportWallchart(models.TransientModel):
    ...
    def check_report(self):
        self.ensure_one()
        if not (
            self.mo or self.tu or self.we or self.th or self.fr or self.sa or self.su
        ):
            raise UserError(self.env._("You must check at least one day."))
        data = {}
        [data] = self.read()
        data["ids"] = self.env.context.get("active_ids", [])
        data["model"] = self.env.context.get(
            "active_model", "report.wallchart"
        )  # 'ir.ui.menu')
        data["form"] = self.read(["mo", "tu", "we", "th", "fr", "sa", "su"])[0]

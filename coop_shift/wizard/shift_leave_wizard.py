...
class ShiftLeaveWizard(models.TransientModel):
    ...
    def _default_shift_template_registration_line_ids(self):
        line_ids = []
        leave_id = self.env.context.get(
            "active_model"
        ) == "shift.leave" and self.env.context.get("active_id", False)
        if leave_id:
    ...
    def button_confirm(self):
        ...
        for registration_id in registration_ids:
            ...
            if existed_line:
                existed_line.with_context(bypass_leave_change_check=True).write(
                    {"state": "waiting", "leave_id": leave.id}
                )
            else:

...
class ShiftTemplate(models.Model):
    ...
    shift_qty = fields.Integer(string="Number of shifts", compute="_compute_shift_qty")
    user_id = fields.Many2one("res.partner", string="Shift Leader")
    ...
    start_datetime = fields.Datetime(
        ...
        required=True,
        help="First date this shift" " will be scheduled",
    )
    start_datetime_tz = fields.Datetime(
        string="Start Date Time (tz)", compute="_compute_start_datetime_tz", store=True
    )
    end_datetime = fields.Datetime(
        string="End Date Time", required=True, help="End date of the first" "  shift"
    )
    end_datetime_tz = fields.Datetime(
        string="End Date Time (tz)", compute="_compute_end_datetime_tz", store=True
    )
    start_date = fields.Date(
        string="Obsolete Start Date",
    ...
    start_time = fields.Float(
        string="Obsolete Start Time", compute="_compute_start_time", store=True
    )

    end_time = fields.Float(
        string="Obsolete End Time", compute="_compute_end_time", store=True
    )
    ...
    rrule = fields.Char(
        compute="_compute_rulestring",
        store=True,
        string="Recurrent Rule",
    )

    is_technical = fields.Boolean(default=False)
    is_ftop = fields.Boolean(related="shift_type_id.is_ftop", store=True)
    ...
    def _compute_template_name(self):
        for template in self:
            ...
            if template.company_id and template.address_id:
                if (
                    template.address_id.name
                    and template.address_id.name != template.company_id.name
                ):
                    addr_name = template.address_id.name
                    isa_characters = "".join([c for c in addr_name if c.isalnum()])
                    str_place = udd.normalize("NFKD", isa_characters[0:5]).encode(
                        "ascii", "ignore"
                    )
                    if str_place:
                        name += f" - {str_place}"
    ...
    def _compute_start_datetime_tz(self):
        ...
        for template in self:
            if template.start_datetime:
                utc_timestamp = pytz.utc.localize(template.start_datetime, is_dst=False)
    ...
    def _compute_end_datetime_tz(self):
        ...
        for template in self:
            if template.end_datetime:
                utc_timestamp = pytz.utc.localize(template.end_datetime, is_dst=False)
                context_tz = pytz.timezone(tz_name)
    ...
    def _compute_start_time(self):
        for template in self:
            if template.start_datetime_tz:
                ...
                template.start_time = start_date_object.hour + (
    ...
    def _compute_end_time(self):
        for template in self:
            if template.end_datetime_tz:
                ...
                template.end_time = end_date_object.hour + (
    ...
    def _compute_week_name(self):
        for template in self:
            if template.week_number:
                template.week_name = self._number_to_letters(template.week_number)
            else:
        ...
        for template in self:
            if template.shift_ids:
                template.last_shift_date = max(
                    fields.Datetime.to_string(shift.date_begin)
    ...
    def _check_seats_limit(self):
        for templ in self:
            if (
                templ.seats_availability == "limited"
                and templ.seats_max
    ...
    def write(self, vals):
        ...
        if "user_ids" in vals and "updated_fields" in vals and len(vals.keys()) <= 2:
            ...
            only_update_seats_max = all(
                key in ["seats_max", "updated_fields"] for key in vals.keys()
            )
    ...
    def update_shift(self, vals):
        ...
        if user_ids:
            for record in self:
                if len(record.shift_ids):
                    shifts = record.shift_ids.filtered(
                        lambda s: s.date_end >= fields.Datetime.today()
                    )
            ...
            res.append(
                (
                    ...
                    0,
                    {
                        ...
                        "interval_type": shift_mail.interval_type
                        and shift_mail.interval_type.replace("shift", "event"),
    ...
    def create_shifts_from_template(self, after=False, before=False):
        ...
        for template in self:
            ...
            rec_dates = template.get_recurrent_dates(after=after, before=before)
            for rec_date in rec_dates:
                ...
                date_check = self.env["ir.fields.converter"]._str_to_datetime(
                    None, None, fields.Datetime.to_string(date_begin)
                )[0]
                date_check = fields.Datetime.from_string(date_check).date()

                if template.last_shift_date and date_check <= template.last_shift_date:
                    continue
                end_date_object_tz = template.end_datetime_tz
                diff_day = end_date_object_tz.day - start_date_object_tz.day
                diff_month = end_date_object_tz.month - start_date_object_tz.month
    ...
    def compute_rule_string(self, data):
        ...
        def get_week_string(freq, data):
            ...
            if freq == "weekly":
                byday = map(
                    lambda x: x.upper(),
                    filter(lambda x: data.get(x) and x in weekdays, data),
                )
                if byday:
        ...
        def get_end_date(data):
            if data.get("final_date"):
                ...
                if isinstance(final_date, date):
                    final_date = fields.Date.to_string(final_date)
                data["end_date_new"] = (
                    "".join((re.compile(r"\d")).findall(final_date)) + "T235959Z"
                )
            return (
                data.get("end_type") == "count"
                and (";COUNT=" + str(data.get("count")))
                or ""
            ) + (
                (
                    data.get("end_date_new")
                    and data.get("end_type") == "end_date"
                    and (";UNTIL=" + data.get("end_date_new"))
                )
                or ""
            )
        ...
        if freq:
            interval_srting = (
                data.get("interval")
                and (";INTERVAL=" + str(data.get("interval")))
                or ""
            )
            res = (
                "FREQ="
                + freq.upper()
                + get_week_string(freq, data)
                + interval_srting
                + get_end_date(data)

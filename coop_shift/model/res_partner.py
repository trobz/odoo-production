...
class ResPartner(models.Model):
    ...
    tmpl_reg_ids = fields.One2many(
        "shift.template.registration", "partner_id", "Template Registrations"
    )

    tmpl_reg_line_ids = fields.One2many(
        "shift.template.registration.line", "partner_id", "Template Registration Lines"
    )
            ...
            if partner.display_std_points > 0:
                ...
                raise ValidationError(
                    _(
                        ...
                        + "need to change its type to FTOP."
                    )
                    % partner_name
                )
    ...
    def get_next_registration(self, next_registrations):
        ...
        ftop_next_registrations = next_registrations.filtered(
            lambda r: r.state == "open" and r.shift_id.shift_type_id.is_ftop
        )
        ...
        for registration in next_registrations.filtered(
            lambda r: r.state == "open" and not r.shift_id.shift_type_id.is_ftop
        ):
            if num_abcd < 4:
    ...
    def _compute_registration_counts(self):
        ...
        for partner in self:
            next_registrations = partner.sudo().registration_ids.filtered(
                lambda r, d=d: r.date_begin >= d and r.state != "cancel"
            )

            # Set 4 next shifts
            next_vals = partner.sudo().get_next_registration(next_registrations)
            ...
            partner.next_registration_id = (
                next_registrations and next_registrations[0] or False
            )
    ...
    @api.depends(
        "counter_event_ids",
        "counter_event_ids.point_qty",
        "counter_event_ids.type",
        ...
        for partner in self:
            partner.final_standard_point = sum(
                [
                    ...
                    if point_counter.type == "standard" and not point_counter.ignored
                ]
            )
        ...
        for partner in self:
            partner.final_ftop_point = sum(
                [
                    point_counter.point_qty
                    for point_counter in partner.counter_event_ids
                    if point_counter.type == "ftop" and not point_counter.ignored
                ]
            )
    ...
    def _compute_display_counter_point(self):
        ...
        for partner in self:
            partner.display_std_points = sum(
                [
                    point_counter.point_qty
                    for point_counter in partner.counter_event_ids
            ...
            partner.display_ftop_points = sum(
                [
                    point_counter.point_qty
                    for point_counter in partner.counter_event_ids
    ...
    def _compute_date_alert_stop(self):
        ...
        alert_duration = int(
            ...
            .sudo()
            .get_param("coop.shift.state.delay.duration")
        )
        ...
        current_partner_alert_date = {
            ...
            for d in self.search_read([("id", "in", self.ids)], ["date_alert_stop"])
        }
        ...
        for partner in self:
            ...
            if point >= 0:
                ...
                partner.date_alert_stop = fields.Date.today() + relativedelta(
                    days=alert_duration
                )

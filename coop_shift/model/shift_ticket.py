...
class ShiftTicket(models.Model):
    ...
    user_id = fields.Many2one("res.partner", related="shift_id.user_id", store=True)

    hide_in_member_space = fields.Boolean(
        "Masquer dans l'Espace Membre",
        default=False,
    ...
    def _compute_begin_date_fields(self):
        for ticket in self:
            ticket.begin_date_string = (
                ticket.date_begin
                and fields.Datetime.to_string(ticket.date_begin + timedelta(hours=2))
                or False
            )

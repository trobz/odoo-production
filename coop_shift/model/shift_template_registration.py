...
class ShiftTemplateRegistration(models.Model):
    ...
    shift_ticket_id = fields.Many2one(
        ...
        "Shift Ticket",
        required=True,
        default=lambda rec: rec._get_default_ticket(),
        copy=True,
    )
    ...
    line_ids = fields.One2many(
        ...
        "registration_id",
        string="Lines",
        default=lambda rec: rec._default_lines(),
        copy=True,
    )
    state = fields.Selection()
    template_start_date = fields.Date(
        string="Template Start Date",
        related="shift_template_id.start_date",
        readonly=True,
    )
    template_start_time = fields.Float(
        string="Template Start Time",
    ...
    def _check_registration_type(self):
        for rec in self:
            ...
            ) and rec.shift_ticket_product_id != rec.env.ref(
    ...
    def _get_default_ticket(self):
        ...
        if active_id:
            template = self.env["shift.template"].browse(active_id)
            if template.shift_type_id == self.env.ref("coop_shift.shift_type"):
                return (
                    template.shift_ticket_ids.filtered(
                        lambda t, s=self: t.product_id
                        == s.env.ref("coop_shift.product_product_shift_standard")
                    )[0]
                    or False
                )
            else:
                return (
                    template.shift_ticket_ids.filtered(
                        lambda t, s=self: t.product_id
                        == s.env.ref("coop_shift.product_product_shift_ftop")
                    )[0]
                    or False
                )
        else:
    ...
    def onchange_shift_id(self):
        ...
        for reg in self:
            shift_ticket_ids = reg.shift_template_id.shift_ticket_ids.filtered(
                lambda t: t.product_id == standard_product
            )
            if shift_ticket_ids:

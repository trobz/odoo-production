    ...
    def _get_shifts(self, date_report, shifts):
        if shifts:
            ...
            date2 = datetime.datetime.strftime(
                datetime.datetime.strptime(date_report, "%Y-%m-%d") + timedelta(days=1),
                "%Y-%m-%d",
            )
        ...
        for shift in shifts:
            ...
            for reg in shift.registration_ids:
                ...
                is_on_leave = self.check_partner_on_leave(reg.partner_id, date_report)
                ...
                if reg.shift_ticket_id.product_id == self.env.ref(
                    ...
                    if (
                        reg.state not in ("replacing", "replaced")
                        and reg.exchange_state != "in_progress"
                    ):

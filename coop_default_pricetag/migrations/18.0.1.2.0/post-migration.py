...
def migrate(cr, version):
    """Fix barcode URLs in all views.

    Replaces /report/barcode/?type= with /report/barcode/?barcode_type=.
    """
    if not version:
        return
    ...
    for view_id, arch_db in barcode_results:
        ...
        for lang_code, arch_content in arch_dict.items():
            ...
            if barcode_pattern_old in arch_content:
                updated_content = arch_content.replace(
                    barcode_pattern_old, barcode_pattern_new
                )
                arch_dict[lang_code] = updated_content
                modified = True
                _logger.info(
                    "Found barcode URL pattern in view (id=%s, lang=%s),"
                    " replacing with new parameter name",
                    view_id,
                    lang_code,
                )

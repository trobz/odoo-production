...
``coop_default_pricetag`` extends the printing flow from
``product_print_category`` to print price tags (pricetags) for products
using QWeb reports.
...
products still to print, depending on ``product_print_category``
configuration).
...
3. Use the print action from ``product_print_category`` (opens the
   *Print Wizard*).

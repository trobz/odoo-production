...
class ShiftLeave(models.Model):
    ...
    provided_birth_certificate = fields.Boolean(string="Birth Certificate Provided")
    shared_leave = fields.Boolean()
    ...
    state = fields.Selection(
        selection_add=[("not_finished", "Not finished")],
        ondelete={"not_finished": "cascade"},
    )
    ...
    def compute_is_parental_leave(self):
        leave_type_name = (
            ...
            .sudo()
            .get_param("coop_parental_leave.leave_type_name")
        )
        ...
        for leave in self:
            leave.is_parental_leave = (
                leave.type_id and leave.type_id == parental_shift_type
            )
    ...
    def _compute_forced_member_status(self):
        ...
        for leave in self:
            forced_member_status = False
            if (
                leave.is_parental_leave
                and leave.expected_birthdate
    ...
    def send_validated_parental_leave_email(self):
        ...
        if self.is_parental_leave:
            ...
            today_dt = fields.Date.today()
            if (
                self.expected_birthdate < self.start_date

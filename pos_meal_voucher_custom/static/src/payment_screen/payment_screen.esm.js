...
patch(PaymentScreen.prototype, {
    ...
    async addNewPaymentLine(paymentMethod, manualMealVoucher = false) {
        ...
        if (
            this.auto_apply_meal_voucher_amount(paymentMethod, manualMealVoucher)
        ) {
            const mealVoucherPayments = this.currentOrder.payment_ids.filter((paymentLine) =>
    ...
    apply_meal_amount(paymentLine) {
        paymentLine.set_amount(Math.max(
            0,
            Math.min(
                this.mealVoucherEligibleAmount,
                this.maxMealVoucherAmount,
            )
        ));
        return;
    },

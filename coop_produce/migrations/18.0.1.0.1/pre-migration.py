...
def migrate(cr, version):
    ...
    if column_exists(cr, "product_product", "default_packaging"):
        _logger.info(
            "Column product_product.default_packaging already exists, skipping."
        )
    else:
        _logger.info(
            "Creating column product_product.default_packaging (float) before upgrade."
        )
        cr.execute("ALTER TABLE product_product ADD COLUMN default_packaging float")
    ...
    if column_exists(cr, "stock_quant", "packaging_qty"):
        ...
        _logger.info(
            "Creating column stock_quant.packaging_qty (float) before upgrade."
        )

...
class ShiftShift(models.Model):
    ...
    css_color_style = fields.Char(compute="_compute_css_color_style")
    ...
    def _compute_css_color_style(self):
        ...
        for shift in self:
            css_color_style = ""
            if (
                shift.ticket_seats_available > 0
                and shift.date_begin >= today
                and shift.date_begin <= tomorrow
            ):
    ...
    def _compute_ticket_seats_shift(self):
        ...
        for shift in self:
            ...
            if shift.shift_ticket_ids:
                ...
                ticket_seats_available = sum(shift_tickets.mapped("seats_available"))
                ticket_seats_max = sum(shift_tickets.mapped("seats_max"))
            shift.ticket_seats_available = ticket_seats_available
            shift.ticket_seats_max = ticket_seats_max

    def get_coordinators(self, get_alias_coordinator=False):
        self.ensure_one()
    ...
    def fetch_ftop_ticket(self, shift_id):
        msg = ""
        args = [("shift_id", "=", shift_id), ("shift_type", "=", "ftop")]

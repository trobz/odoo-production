...
class InvoiceSupplierPriceUpdate(models.TransientModel):
    ...
    def compute_process_lines(self, active_model, active_obj):
        """
        update product prices/order line prices based on latest EDI supplier \
        prices (Only for EDI suppliers)
        ...
        for line in obj_lines:
            ...
            selected_seller_id = product_id.seller_ids.filtered(
                lambda seller: seller.name == partner_id and
                seller.price_policy == line.price_policy
            )
            selected_seller_id = \
            ...
            if selected_seller_id:
                ...
                if partner_id.is_edi:
                    ...
                    edi_price = supplier_price_list_obj.search(
                        price_domain,
    ...
    def update_prices_second(self):
        ...
        for line in lines:
            ...
            if active_model == 'account.invoice':
                obj_lines_values.append((1, line.invoice_line_id.id, {
                    'price_unit': updated_price_unit
                }))
            elif active_model == 'purchase.order':
                obj_lines_values.append((1, line.po_line_id.id, {
                    'price_unit': updated_price_unit
                }))
        if active_model == 'account.invoice':
...
class SupplierInfoUpdateLine(models.TransientModel):
    ...
    inv_supplier_price_id = fields.Many2one(
        comodel_name='invoice.supplier.price.update'
    )

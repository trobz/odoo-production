...
odoo.define("pos_scrap_order.screens", function (require) {
    ...
    var ScrapScreenWidget = screens.ScreenWidget.extend({
        ...
        render_scrap: function () {
            this.$(".pos-scrap-container").html(
                QWeb.render("ScrapLines", this.get_scrap_render_env())
            );
        },
    });
    ...
    var ScrapListScreenWidget = screens.ScreenWidget.extend({
        ...
        show: function () {
            this._super();
            self = this;
            this.fetch_scrap_orders()
                .then(function (scrapOrders) {
        ...
        render_list: function (scrapOrders) {
            ...
            for (var i = 0, len = scrapOrders.length; i < len; i++) {
                var scrapLine_html = QWeb.render("ScrapListLine", {
                    widget: this,
                    scrapOrder: scrapOrders[i],
                });

...
class ResPartner(models.Model):
    ...
    amount_subscription = fields.Float(store=True)
    coop_amount = fields.Float(
        compute="_compute_coop_amount", 
        store=True
    )
    ...
    @api.depends(
        "fundraising_invoice_ids", "fundraising_invoice_ids.state",
        "fundraising_invoice_ids.invoice_line_ids"
    )
    def _compute_amount_subscription(self):
        super()._compute_amount_subscription()

    @api.depends(
        "fundraising_journal_item_ids", "fundraising_journal_item_ids.move_id.state",
        "fundraising_journal_item_ids.account_id","fundraising_journal_item_ids.partner_id",
    )

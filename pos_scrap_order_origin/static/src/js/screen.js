...
odoo.define("pos_scrap_order_origin.screens", function(require) {
    ...
    scrapScreens.ScrapScreenWidget.include({
        ...
        _get_vals: function(order_vals){
            if (this.pos.config.scrap_origin_ids.length === 0){
                this.gui.show_popup("error", {
                    ...
                    "body":  _t(
                        "Veuillez configurer l'origine du rebut dans le backend.")
                });
            }
            else {
                ...
                if (origin.length < 1){
                    this.gui.show_popup("error", {
                        "title": _t("Pas d'origine"),
                        "body":  _t(
                            "Veuillez sélectionnez l'origine.")
                    });
                }
                else {
        ...
        render_scrap: function() {
            ...
            this.$('.pos-scrap-left-container').html(QWeb.render('ScrapScreen-Origins', {scrapOrigins: scrapOrigins}));

...
class CapitalFundraisingCategory(models.Model):
    ...
    company_id = fields.Many2one(
        ...
        string="Company",
        related="fundraising_id.company_id",
        store=True,
        readonly=True,
    )
    ...
    refund_account_id = fields.Many2one(
        comodel_name="account.account", string="Refund Account"
    )
    ...
    line_ids = fields.One2many(
        comodel_name="capital.fundraising.category.line",
        string="Lines",
        inverse_name="fundraising_category_id",
    )
    journal_refund_ids = fields.Many2many(
        comodel_name="account.journal",
        string="Journals for Capital Refund",
        ...
        if partner.fundraising_partner_type_ids:
            for line in category.line_ids:
                if (
                    ...
                    in partner.fundraising_partner_type_ids.ids
                ):
    ...
    def get_deficit_share_amount(self, date_invoice):
        ...
        for deficit_amount in self.deficit_share_amount_ids:
            if (
                not deficit_amount.start_date
                or deficit_amount.start_date <= date_invoice
            ) and (
                not deficit_amount.end_date or deficit_amount.end_date >= date_invoice
            ):

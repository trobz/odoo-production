...
class CapitalFundraisingDeficit(models.Model):
    ...
    amount_by_share = fields.Float("Amount By Share", required=True, digits=(6, 6))
        ...
        for deficit_line in self:
            ...
            if end_date and start_date > end_date:
                raise ValidationError(
                    _("Stop Date should be greater than " "Start Date.")
                )

            # Searching for deficit of the same category
            same_categ_deficits = self.search(
                [("fund_cate_id", "=", fund_cate.id), ("id", "!=", deficit_line.id)]
            )
            ...
            for check_deficit in same_categ_deficits:
                ...
                if not deficit_line.end_date:
                    if (
                        not check_deficit.end_date
                        or check_deficit.end_date >= deficit_line.start_date
                    ):
                        is_overlap = True
                else:
                    if (
                        not check_deficit.end_date
                        and check_deficit.start_date <= deficit_line.end_date
                    ) or (
                        check_deficit.end_date
                        and check_deficit.end_date >= deficit_line.start_date
                        and check_deficit.start_date <= deficit_line.end_date
                    ):
                        is_overlap = True
                if is_overlap:

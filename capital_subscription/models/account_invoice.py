...
class AccountInvoice(models.Model):
    ...
    def _prepare_refund(
        ...
        journal_refund_ids = invoice.fundraising_category_id.journal_refund_ids.ids
    ...
    def _check_capital_fundraising(self):
        ...
        if invoice.is_capital_fundraising:
            ...
            if not invoice.fundraising_category_id:
                raise exceptions.UserError(
                    _("A Capital fundraising must have a capital category" " defined")
                )

            # Check products
            forbidden_product_ids = list(
                set(product_ids) - set([invoice.fundraising_category_id.product_id.id])
            )
            if forbidden_product_ids:
                ...
                if not all(
                    [each_prod.is_deficit_product for each_prod in forbidden_products]
                ):
                    raise exceptions.UserError(
                        _("%s category do not allow %s products")
                        % (
                            invoice.fundraising_category_id.name,
                            ", ".join(forbidden_products.mapped("name")),
                        )
                    )
        ...
        if invoice.state in ["open", "paid"] and invoice.fundraising_category_id:
    ...
    def onchange_fundraising_category_id(self):
        if self.fundraising_category_id:
            ...
            if self.fundraising_category_id.partner_account_id:
                self.account_id = self.fundraising_category_id.partner_account_id
    ...
    def apply_refund_deficit_share(self, quantity=0):
        ...
        for invoice in self:
            ...
            if invoice.type == "out_refund" and fundraising_categ and quantity >= 0:
                    ...
                    if source_product and source_product.is_capital_fundraising:
                        ...
                        if deficit_share_amount:
                            ...
                            deficit_line_val = {
                                ...
                                or _("Deficit Share"),
                                "account_id": source_product.deficit_share_account_id.id,
                                "quantity": quantity,
                                "price_unit": deficit_price_unit_signed,
                                "uom_id": inv_line.uom_id
                                and inv_line.uom_id.id
                                or False,
                                "invoice_id": invoice.id,
                            }

                            self.env["account.invoice.line"].create(deficit_line_val)

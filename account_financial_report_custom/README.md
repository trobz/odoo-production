...
## Overview
This module provides customizations for OCA `account_financial_report` reports (Odoo 18), focused on improving the **General Ledger** output.
...
## Key Features
- Custom regrouping of General Ledger move lines to avoid duplicated / overly granular lines in the report.
  ...
  - Move lines are grouped by:
    ...
    - Currency
- QWeb template adjustment for General Ledger lines (`report/templates/general_ledger.xml`).
  - Displays taxes, analytic distribution (cost center), analytic tags, matching number, and optional foreign currency columns.
...
- The report layout customization is done via QWeb inheritance in `report/templates/general_ledger.xml`.

...
class ShiftLeave(models.Model):
    ...
    def send_mail_reminder_non_defined_leaves(self):
        ...
        # update un_subscribed for members exceeds end_date of non-definied leaves

        leave_to_update = leave_env.search([
            ('is_send_reminder', '=', True),
            ('non_defined_leave', '=', True),
            ('partner_id.working_state', 'in', ['on_vacation', 'extempted']),
            ('stop_date', '=',
                (datetime.now() - timedelta(days=1)).strftime('%Y-%m-%d')),
        ])
        
        for leave in leave_to_update:
            next_template = leave.partner_id.tmpl_reg_line_ids.filtered(
                lambda t: t.is_future or t.is_current)
            if not next_template:
                leave.partner_id.cooperative_state = 'unsubscribed'

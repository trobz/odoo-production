...
class ShiftRegistration(models.Model):
    ...
    def adjust_qty_on_holiday(self, vals):
        ...
        if vals in ['absent', 'excused']:
            counter_vals = {}
            if self.shift_type == 'standard':
                if (single_holiday and self.shift_id.state_in_holiday == 'closed')\
                        or (long_holiday and long_holiday.make_up_type == '0_make_up'):
                    if vals in ['absent']:
                        counter_vals.update({
                            'point_qty': 2
                        })
                ...
                elif (single_holiday and self.shift_id.state_in_holiday == 'open')\
                        or (long_holiday and long_holiday.make_up_type == '1_make_up'):
                    if vals in ['absent']:
                        counter_vals.update({
                            'point_qty': 1
                        })
    ...
    @api.multi
    def balance_point_qty_ftop_shift(self, holiday_id, current_point):
        self.ensure_one()
        point_counter_env = self.env['shift.counter.event']
        if self.state in ['absent', 'excused']:
            if current_point >= 1:
                point = 1
            else:
                point = 2
    ...
    @api.multi
    def adjust_qty_for_ftop_on_holiday(self, holiday_id, att_state):
        self.ensure_one()
        point_counter_env = self.env['shift.counter.event']
        if self.state in ['absent', 'excused'] and\
                att_state in ['0_make_up', 'closed']:
            point = 1
            # Create Point Counter
            point_counter_env.sudo().with_context(
                {'automatic': True}).create({
                    'name': _('Balance Qty For FTOP Shift Cloture'),
                    'shift_id': self.shift_id.id,
                    'type': 'ftop',
                    'partner_id': self.partner_id.id,
                    'point_qty': point,

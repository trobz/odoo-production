...
class ShiftChangeTeam(models.Model):
    ...
    def _compute_name_change_team(self):
        for record in self:
            record.name = '%s Changes Team' % (record.partner_id.name)
        ...
        for record in self:
            if record.partner_id:
                ...
                if record.partner_id.is_unsubscribed:
                    record.partner_state = 'unsubscribed'
                    record.show_partner_state = True
                    if record.is_catch_up:
                        record.is_catch_up = False
                else:
                    record.partner_state = 'subscribed'
                    record.show_partner_state = False
    ...
    def compute_range_day(self):
        ...
        if new_team_start_date > 3:
            new_next_shift_date = (fields.Datetime.from_string(
                self.new_next_shift_date) - timedelta(days=3)).strftime('%Y-%m-%d')
        ...
        for record in self:
            if record.next_current_shift_date and record.new_shift_template_id\
                ...
                if not record.current_shift_template_id.shift_type_id.is_ftop\
                    ...
                    if range_dates and range_dates <= 14:
                        record.mess_change_team = (_(

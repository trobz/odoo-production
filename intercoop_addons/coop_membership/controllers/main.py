...
class WebsiteRegisterMeeting(http.Controller):
    ...
    def subscribe_discovery_meeting(self, **post):
        ...
        event_name = post.get('select_event', False)
        dob = post.get('dob', False)

        # conver dob to correct format in database
        try:
            dob = datetime.strptime(dob, "%m/%d/%Y").date().strftime('%Y-%m-%d')
        except:
            _logger.warn('Convert birthdate from %s on discovery meeting form failed', dob)

        # Get id event from event name
        if event_name:
            event_id = int(event_name[:1])
        else:
            event_id = False
        ...
        event_obj = request.registry['event.event']
        event = event_obj.browse(request.cr, request.uid, event_id,
                                 context=request.context)
